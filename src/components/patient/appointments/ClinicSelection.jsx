import { getClinics } from "@/config/firestore";
...
const ClinicSelection = ({
  ...
  useEffect(() => {
    const fetchClinics = async () => {
      try {
        const data = await getClinics(province, city); // Or get province and city from user input
        setClinics(data);
        console.log(data);
      } catch (error) {
        console.log(error);
  ...
  return (
    <div className={styles.formSection}>
      <h2 className={styles.sectionTitle}>
        <i className={`${styles.icon} ${styles.clinicIcon}`}>🏥</i> Clinic
        Details
      </h2>
